...
public:
    EventListener(const Rml::String& value, Rml::Element* element) : value(value), element(element) {}
...
{
public:
    Rml::EventListener* InstanceEventListener(const Rml::String& value, Rml::Element* element) override { return new EventListener(value, element); }
};
...
public:
    ...
    bool LoadTexture(Rml::TextureHandle& texture_handle, Rml::Vector2i& texture_dimensions, const Rml::String& source) override { return true; }
};
...
void Backend::RequestExit() { RMLUI_ASSERT(data); }
...
void gui_system_begin(void) { Backend::BeginFrame(); }

void gui_system_end(void) { Backend::PresentFrame(); }

gui_window_t::gui_window_t(std::string name, int width, int height) : m_name(name)
{
...
void gui_window_t::events(SDL_Event* event)
{
    ...
    switch (event->type)
    {
        ...
        case SDL_KEYDOWN:
            if (!RmlSDL::InputEventHandler(m_context, *event)) break;
            break;

        default: RmlSDL::InputEventHandler(m_context, *event); break;
    }
}

void gui_window_t::ProcessEvent(Rml::Event& event) { process_event(event); }
...
void gui_window_events(gui_window_t* gui, SDL_Event* event) { gui->events(event); }

void gui_window_update(gui_window_t* gui) { gui->update(); }

void gui_window_render(gui_window_t* gui) { gui->render(); }

void gui_window_resize(gui_window_t* gui, float width, float height) { gui->resize(width, height); }
